package me.whereareiam.spawner.platform.target.proxy.bungeecord.config
...
open class BungeeCordScenarioInstanceConfig @Inject constructor(
	instanceName: String,
	...
	val ipForward: Property<Boolean> = objects.property(Boolean::class.java).convention(true)
	val tryServers: ListProperty<String> = objects.listProperty(String::class.java).convention(emptyList())

	private val servers = linkedMapOf<String, BungeeCordServerEntryConfig>()

	fun server(name: String, address: String) {
		val entry = servers.getOrPut(name) {
			objects.newInstance(BungeeCordServerEntryConfig::class.java, name)
		}
		entry.address(address)
	}

	fun server(name: String, action: Action<in BungeeCordServerEntryConfig>) {
		val entry = servers.getOrPut(name) {
			objects.newInstance(BungeeCordServerEntryConfig::class.java, name)
		}
	...
	fun servers(): Collection<BungeeCordServerEntryConfig> = servers.values
}
